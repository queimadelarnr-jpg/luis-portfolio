...
(() => {
    ...
    const fail = () => {
        ...
        links.className = "cube-fallback-links";
        ["about", "drip", "hindo", "connect"].forEach(name => {
            const link = document.createElement("button");
            link.type = "button";
            link.textContent = name.toUpperCase();
            link.onclick = () => { const project = name === "drip" || name === "hindo"; switchPage(project ? "work" : name); if (project)
                document.querySelectorAll(".project-detail").forEach(detail => detail.classList.toggle("is-open", detail.dataset.project === name)); backButton.hidden = false; };
            links.appendChild(link);
        });
        fallback.appendChild(links);
    ...
    /* =========================================
       3D SCENE — camera, lighting and materials
    ========================================= */
    ...
    // Artwork wraps the same rounded surface, including its bevel and edges.
    // A removable skin lets game mode reveal the original coloured sticker.
    const artworkBackingMaterial = new T.MeshStandardMaterial({ visible: false });
    const artworkGeometry = stickerGeometry.clone();
    const artworkPositions = artworkGeometry.attributes.position;
    const artworkUV = artworkGeometry.attributes.uv;
    for (let i = 0; i < artworkPositions.count; i++) {
        artworkUV.setXY(i, (artworkPositions.getX(i) + .405) / .81, (artworkPositions.getY(i) + .405) / .81);
    }
    artworkUV.needsUpdate = true;
    const faces = [
    ...
    /* =========================================
       TILE CONTENT — labels, images and destinations
    ========================================= */
    ...
    /* =========================================
       TILE ARTWORK — draw images and text onto a texture
    ========================================= */
    ...
    function tileTexture(item) {
        ...
        function paint(image) {
            ctx.fillStyle = item.section === "connect" ? "#EE9134" : "#EFEDEA";
            ...
            if (image) {
                ...
                ctx.fillText(item.section.toUpperCase() + " \u2197", 26, 475);
            }
            else {
                ...
                if (item.interactive !== false)
                    ctx.fillText(item.action ? (item.action === ".connect-cv" ? "DOWNLOAD \u2193" : "OPEN \u2197") : "EXPLORE \u2197", 26, 475);
            }
        ...
        if (item.image) {
            ...
            image.onload = () => { paint(image); wake(); };
            image.src = window.CUBE_IMAGES?.[item.image] || item.image;
        }
        return texture;
    }
        ...
        for (let y = -1; y <= 1; y++)
            for (let z = -1; z <= 1; z++) {
                ...
                faces.forEach((face, i) => {
                    if (piece.position[face.axis] !== face.sign)
                        return;
                    ...
                    if (tileKey) {
                        ...
                        if (!textures.has(tileKey))
                            textures.set(tileKey, tileTexture(item));
                        const art = new T.Mesh(artworkGeometry, new T.MeshStandardMaterial({ map: textures.get(tileKey), roughness: .48 }));
                        // Render exactly one surface. Hiding the backing material keeps
                        // the artwork child visible; game mode swaps in its own material.
                        sticker.material = artworkBackingMaterial;
                        sticker.add(art);
                        if (item.interactive !== false)
                            tiles.push({ sticker, art, item, face: i });
                    }
                });
                cube.add(piece);
                pieces.push(piece);
    ...
    function advance(direction) {
        if (!interactive() || section !== "home" || faceTurn || turn || step + direction < 0 || step + direction > sequence.length)
            return;
    ...
    function draw(time) {
        ...
        if (faceTurn) {
            ...
            if (t === 1) {
                ...
                destination = null;
                faceTurn = null;
                home.dataset.cubeSection = section;
                updateCaption();
                updateInstructions();
        ...
        positionTiles();
        positionCallouts();
        if (!motion.matches || turn || faceTurn || destination)
            frame = requestAnimationFrame(draw);
    }
    function wake() { if (active() && !frame)
        frame = requestAnimationFrame(draw); }
    function resize() {
    ...
    stage.addEventListener("pointermove", event => {
        if (gameMode || event.pointerType === "touch")
            return;
        const box = stage.getBoundingClientRect();
        pointer = { x: (event.clientX - box.left) / box.width * 2 - 1, y: (event.clientY - box.top) / box.height * 2 - 1 };
        wake();
    });
    stage.addEventListener("pointerleave", () => { pointer = { x: 0, y: 0 }; });
    home.addEventListener("wheel", event => {
        if (destination || !interactive() || event.ctrlKey || Math.abs(event.deltaX) > Math.abs(event.deltaY))
            return;
        event.preventDefault();
        const now = performance.now();
        const delta = event.deltaY * (event.deltaMode === 1 ? 16 : event.deltaMode === 2 ? stage.clientHeight : 1);
        if (now - lastWheel > 180 || Math.sign(delta) !== Math.sign(wheelTotal))
            wheelTotal = 0;
        lastWheel = now;
        if (turn) {
            wheelTotal = 0;
        ...
        wheelTotal += delta;
        if (Math.abs(wheelTotal) >= 80) {
            scrollStep(Math.sign(wheelTotal));
            wheelTotal = 0;
        }
    }, { passive: false });
    ...
    autoButton.addEventListener("click", () => { if (section === "home" && !destination && !turn)
        navigate("home"); });
    let touch = null;
    ...
    stage.addEventListener("keydown", event => {
        if (event.key === "ArrowDown" || event.key === "ArrowUp") {
            event.preventDefault();
            scrollStep(event.key === "ArrowDown" ? 1 : -1);
        }
    });
    buttons.forEach(button => button.addEventListener("click", () => advance(Number(button.dataset.cubeStep))));
    if (matchMedia("(pointer:coarse)").matches)
        home.querySelector(".cube-instruction").textContent = "Swipe up to solve \u00B7 down to rewind \u00B7 or tap the arrows";
    ...
    tileLayer.className = "cube-tile-layer";
    stage.appendChild(tileLayer);
    const raycaster = new T.Raycaster();
    tiles.forEach((tile, index) => {
        const button = document.createElement("button");
        button.type = "button";
        button.className = "cube-tile";
        button.setAttribute("aria-label", tile.item.title);
        button.dataset.tile = index;
        const preview = document.createElement("span");
        preview.className = "cube-tile-preview";
        if (tile.item.image) {
            const img = document.createElement("img");
            img.src = tile.item.image;
            img.alt = "";
            preview.appendChild(img);
        }
        else {
            const text = document.createElement("span");
            text.className = "cube-preview-type";
            text.textContent = tile.item.text.replaceAll(" / ", "\n");
            preview.appendChild(text);
        }
        if (tile.item.previewText) {
            const bio = document.createElement("span");
            bio.className = "cube-preview-bio";
            bio.textContent = tile.item.previewText;
            preview.appendChild(bio);
        }
        const label = document.createElement("span");
        label.className = "cube-preview-label";
        label.textContent = tile.item.title + " \u2197";
        preview.appendChild(label);
        button.appendChild(preview);
        tileLayer.appendChild(button);
        tile.button = button;
        button.addEventListener("pointerenter", event => { if (event.pointerType !== "touch")
            showPreview(tile); });
        button.addEventListener("pointerleave", event => { if (event.pointerType !== "touch")
            clearPreview(); });
        ...
        button.addEventListener("click", event => {
            ...
            if (!interactive() || turn || destination || performance.now() < suppressClickUntil)
                return;
            if (touchPress && !alreadyPreviewed) {
                showPreview(tile);
                return;
            }
            clearPreview();
            if (section === "home" || tile.item.section !== section)
                navigate(tile.item.section);
            else if (tile.item.game)
                window.PortfolioCubeGame.open();
            else if (tile.item.action)
                document.querySelector(tile.item.action)?.click();
            else
                openContent(tile.item);
        });
    });
    ...
    calloutLayer.className = "cube-callouts";
    calloutLayer.hidden = true;
    const svg = document.createElementNS("http://www.w3.org/2000/svg", "svg");
    svg.setAttribute("aria-hidden", "true");
    calloutLayer.appendChild(svg);
    stage.appendChild(calloutLayer);
    const callouts = [
        ...
        label.type = "button";
        label.className = "cube-callout";
        label.textContent = item.title;
        label.addEventListener("click", () => navigate(tile.item.section));
        const line = document.createElementNS("http://www.w3.org/2000/svg", "polyline");
        const outline = line.cloneNode();
        outline.classList.add("cube-pointer-outline");
        const dot = document.createElementNS("http://www.w3.org/2000/svg", "circle");
        dot.setAttribute("r", "4");
        svg.append(outline, line, dot);
        calloutLayer.appendChild(label);
        return { ...item, tile, label, line, outline, dot };
    });
    /* =========================================
       HOME LABELS — project tile positions onto the screen
    ========================================= */
    ...
    function positionCallouts() {
        ...
        callouts.forEach(item => {
            ...
            item.outline.setAttribute("points", item.line.getAttribute("points"));
            item.dot.setAttribute("cx", tx);
            item.dot.setAttribute("cy", ty);
        });
    }
    function showPreview(tile) {
        if (turn || destination || modalPage)
            return;
        if (previewTile)
            previewTile.button.classList.remove("is-preview");
        previewTile = tile;
        tile.button.classList.add("is-preview");
    }
    function clearPreview() { if (previewTile)
        previewTile.button.classList.remove("is-preview"); previewTile = null; }
    /* =========================================
       CLICK TARGETS — align HTML buttons with the 3D tiles
    ========================================= */
    ...
    function positionTiles() {
        ...
        tiles.forEach(tile => {
            ...
            let hit = false;
            if (visible) {
                raycaster.set(camera.position, towardCamera.clone().negate());
                const surface = raycaster.intersectObjects(cube.children, true)[0]?.object;
                hit = surface === tile.art || surface === tile.sticker;
            }
            tile.button.hidden = !visible || !hit;
            if (tile.button.hidden) {
                if (previewTile === tile)
                    clearPreview();
                return;
            }
            ...
            const cardWidth = Math.min(Math.max(180, w * 1.65), 280, Math.max(1, window.innerWidth - 24), Math.max(1, window.innerHeight - 24));
    ...
    /* =========================================
       CONTENT DIALOG — project pages and Back to cube
    ========================================= */

    const dialog = document.createElement("dialog");
    dialog.className = "cube-content-dialog";
    dialog.setAttribute("aria-label", "Portfolio content");
    const close = document.createElement("button");
    close.type = "button";
    close.className = "cube-content-close";
    close.textContent = "\u2190 BACK TO CUBE";
    const contentHost = document.createElement("div");
    contentHost.className = "cube-content-host";
    const toolbar = document.createElement("div");
    toolbar.className = "cube-content-toolbar";
    const projectVisit = document.createElement("a");
    projectVisit.className = "cube-content-visit";
    projectVisit.textContent = "VISIT PROJECT \u2197";
    projectVisit.target = "_blank";
    projectVisit.rel = "noopener noreferrer";
    projectVisit.hidden = true;
    toolbar.append(close, projectVisit);
    dialog.append(toolbar, contentHost);
    document.body.appendChild(dialog);
    let placeholder = null, returnFocus = null;
    function openContent(item) {
        const page = document.querySelector(`.page[data-page="${item.section}"]`);
        if (!page)
            return;
        projectVisit.hidden = true;
        projectVisit.removeAttribute("href");
        if (item.section === "work") {
            const detail = Array.from(page.querySelectorAll(".project-detail")).find(el => el.dataset.project === item.project);
            const source = detail?.querySelector("a.visit");
            if (source) {
                projectVisit.href = source.href;
                projectVisit.hidden = false;
                projectVisit.setAttribute("aria-label", `Visit ${item.project} project (opens in a new tab)`);
            }
        }
        returnFocus = document.activeElement;
        clearPreview();
        placeholder = document.createComment("cube content return");
        page.before(placeholder);
        modalPage = page;
        contentHost.appendChild(page);
        page.classList.add("cube-content-page", "page-visible");
        dialog.setAttribute("aria-label", item.title);
        dialog.showModal();
        close.focus();
        if (item.section === "about" && typeof triggerAboutEntrance === "function")
    ...
    function restoreContent() {
        if (!modalPage)
            return;
        if (modalPage.dataset.page === "work")
            modalPage.querySelectorAll(".project-detail").forEach(detail => detail.classList.remove("is-open"));
        modalPage.classList.remove("cube-content-page", "page-visible");
        placeholder.replaceWith(modalPage);
        modalPage = null;
        placeholder = null;
    ...
    stage.addEventListener("keydown", event => { if (event.key === "Escape")
        clearPreview(); });
    home.dataset.cubeSection = "home";
    updateInstructions();
    ...
    function gameEnter() {
        if (gameMode || turn || destination || section !== "work" || broken)
            return;
        clearPreview();
        ...
        const specs = window.PortfolioCubeGameModel.faces;
        if (!gameMaterials)
            gameMaterials = Object.fromEntries(Object.entries(specs).map(([f, s]) => [f, new T.MeshBasicMaterial({ color: s.color })]));
        pieces.forEach(piece => piece.children.slice(1).forEach(sticker => {
            ...
            sticker.material = gameMaterials[face];
            sticker.children.forEach(child => child.visible = false);
        }));
        gameMode = true;
        ...
        gameUI().show();
        presentation.rotation.set(.36, -.52, 0);
        presentation.position.y = 0;
        gameReset(true);
        resize();
        stage.focus({ preventScroll: true });
    }
    function gameExit() {
        ...
        presentation.quaternion.copy(gameSaved.rotation);
        presentation.position.copy(gameSaved.position);
        gameMode = false;
        gameSession = null;
        gameSaved = null;
        pointer = { x: 0, y: 0 };
    ...
    function gamePaintStats() { if (gameSession)
        gameUI().update({ elapsed: gameSession.elapsed(performance.now()), moves: gameSession.moves, busy: !!gameTurn, solved: gameSession.finishedAt !== null }); }
    function gameMove(face, inverse = false) {
        const spec = window.PortfolioCubeGameModel.faces[face];
        if (spec)
            gameLayerMove(spec.axis, spec.layer, spec.sign * (inverse ? -1 : 1), face, inverse);
    }
    function gameLayerMove(axisIndex, layer, sign, face = null, inverse = false) {
        if (!gameMode || gameTurn || broken || gameSession.finishedAt !== null)
            return;
        const axis = ["x", "y", "z"][axisIndex];
        gameSession.start(performance.now());
    ...
    function drawGame(now) {
        if (gameTurn) {
            ...
            if (t === 1) {
                gameTurn.selected.forEach(({ piece }) => { piece.position.round(); piece.quaternion.normalize(); });
                gameSession.commitLayer(["x", "y", "z"].indexOf(gameTurn.axis), gameTurn.layer, gameTurn.sign, now);
                gameTurn = null;
            }
        }
        gamePaintStats();
    ...
    window.addEventListener("portfolio-game-command", event => {
        ...
        if (type === "back")
            ...
            gameMove(face, inverse);
        else if (type === "reset" || type === "shuffle")
            gameReset(type === "shuffle");
        else if (type === "view") {
            presentation.rotation.set(.36, -.52, 0);
            wake();
        }
    ...
    function pickGameTile(x, y) {
        ...
        if (!hit || !gameSaved.surfaces.some(s => s.sticker === hit.object))
            return null;
    ...
    function swipeGameTile(hit, dx, dy) {
        ...
        const origin = project(hit.point);
        let best = null;
    ...
    stage.addEventListener("pointermove", event => {
        if (!gameMode || !gameDrag || gameDrag.id !== event.pointerId)
            return;
        const distance = Math.hypot(event.clientX - gameDrag.x, event.clientY - gameDrag.y);
        if (gameDrag.type === "touch" && !gameDrag.gesture) {
            if (performance.now() - gameDrag.started >= 350)
                gameDrag.gesture = "view";
            else if (distance > 6)
                gameDrag.gesture = gameDrag.hit ? "row" : "ignore";
        }
        if (distance > 6)
            gameDrag.dragged = true;
        ...
        gameDrag.lastX = event.clientX;
        gameDrag.lastY = event.clientY;
        wake();
    });
    ...
    stage.addEventListener("pointerup", event => {
        ...
        if (!gameMode || !gameDrag || event.pointerId !== gameDrag.id)
            return;
        const drag = gameDrag;
        gameDrag = null;
        if (gameTurn)
        ...
        if (drag.type === "touch") {
            ...
            if (drag.gesture !== "view" && drag.gesture !== "ignore" && drag.hit && Math.hypot(dx, dy) >= 18 && (drag.gesture === "row" || performance.now() - drag.started < 350))
                swipeGameTile(drag.hit, dx, dy);
            return;
        }
        if (!drag.dragged && drag.hit)
            gameMove(faceForLayer(drag.hit.axis, drag.hit.layer), event.shiftKey);
    });
    document.addEventListener("keydown", event => {
        ...
        if (face && !event.ctrlKey && !event.metaKey && !event.altKey) {
            event.preventDefault();
            if (!event.repeat)
                gameMove(face, event.shiftKey);
        }
    });
