...
(() => {
    ...
    function build(){
        panel=document.createElement('section');panel.className='cube-game-panel';panel.hidden=true;panel.setAttribute('aria-label','Cube puzzle controls');
        ...
        for(const [key,face] of Object.entries({Q:'L',W:'U',E:'R',A:'F',S:'D',D:'B'})){
            ...
            const row=document.createElement('div');row.className='game-face-control';
            const label=document.createElement('span');label.textContent=spec.name;label.style.setProperty('--face-color',spec.color);row.appendChild(label);
            for(const inverse of [false,true]){const b=document.createElement('button');b.type='button';b.textContent=key+(inverse?' ↶':' ↷');b.className='game-turn';b.setAttribute('aria-label',`${spec.name} ${inverse?'anticlockwise':'clockwise'}`);b.onclick=()=>send('turn',{face,inverse});row.appendChild(b);}
    ...
    window.PortfolioCubeGame={
        open(){if(!panel)build();send('enter');},
        show(){panel.hidden=false;document.querySelector('#home .cube-home').classList.add('game-playing');},
        hide(){if(panel)panel.hidden=true;document.querySelector('#home .cube-home').classList.remove('game-playing');},
        update({elapsed,moves,busy,solved}){
            ...
            panel.querySelector('.game-time').textContent=clock;panel.querySelector('.game-moves').textContent=moves;
            const message=solved?`Solved in ${clock} with ${moves} moves!`:moves||elapsed?'Match all six faces.':'Your first turn starts the timer.';
            const status=panel.querySelector('.game-status');if(status.textContent!==message)status.textContent=message;
