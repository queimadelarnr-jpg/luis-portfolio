...
(function (root) {
    ...
    class Cube {
        ...
        reset(){
            ...
            for(let x=-1;x<=1;x++)for(let y=-1;y<=1;y++)for(let z=-1;z<=1;z++){
                ...
                for(const [face,spec] of Object.entries(faces))if(pos[spec.axis]===spec.layer){
                    const normal=[0,0,0];normal[spec.axis]=spec.layer;stickers.push({face,normal});
                }
                this.pieces.push({pos,stickers});
            }
        }
        turn(face,inverse=false){
            const spec=faces[face];if(!spec)throw new Error('Unknown face');
            this.turnLayer(spec.axis,spec.layer,spec.sign*(inverse?-1:1));
        }
        turnLayer(axis,layer,sign){
            for(const piece of this.pieces)if(piece.pos[axis]===layer){
                piece.pos=rotate(piece.pos,axis,sign);
                for(const sticker of piece.stickers)sticker.normal=rotate(sticker.normal,axis,sign);
            }
        }
        solved(){
            ...
            return this.pieces.every(p=>p.stickers.every(s=>{
                ...
                if(!colors.has(key))colors.set(key,s.face);
    ...
    class Session {
        ...
        shuffle(random=Math.random){
            const keys=Object.keys(faces);this.scramble=[];let lastAxis=-1;
            for(let i=0;i<22;i++){
                const options=keys.filter(k=>faces[k].axis!==lastAxis);
                const face=options[Math.floor(random()*options.length)];
                this.scramble.push({face,inverse:random()<.5});lastAxis=faces[face].axis;
            }
            this.reset();
        }
        reset(){this.cube.reset();for(const m of this.scramble)this.cube.turn(m.face,m.inverse);this.startedAt=null;this.finishedAt=null;this.moves=0;}
        start(now){if(this.finishedAt!==null)return false;if(this.startedAt===null)this.startedAt=now;return true;}
        commit(face,inverse,now){this.cube.turn(face,inverse);this.moves++;if(this.cube.solved())this.finishedAt=now;}
        commitLayer(axis,layer,sign,now){this.cube.turnLayer(axis,layer,sign);this.moves++;if(this.cube.solved())this.finishedAt=now;}
        elapsed(now){return this.startedAt===null?0:Math.max(0,(this.finishedAt??now)-this.startedAt);}
    }
    const api={faces,Cube,Session};
    if(typeof module==='object' && module.exports)module.exports=api;else root.PortfolioCubeGameModel=api;
